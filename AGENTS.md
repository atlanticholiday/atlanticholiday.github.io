...
- Pattern: Cleaning AH cleanings now hide reservation/source controls from the UI and default new manual entries to the standard platform formula internally; keep that simplification in `js/features/operations/cleaning-ah-manager.js` unless the business rules for direct bookings return.
...
- Test: on the Cleanings tab, confirm the entry form no longer shows reservation/source selectors and the stored cleanings table no longer shows Reservation or Source columns, then rerun `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
