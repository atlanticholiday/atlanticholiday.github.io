...
- Pattern: Cleaning AH cleanings now normalize saved rows to category keys in `js/features/operations/cleaning-ah-utils.js`; keep category-specific math (check-out vs owner vs first cleaning vs mid-term) there so forms, filters, and stats stay aligned even when older rows used different category labels.
- Pattern: Laundry handoff tracking now lives in `js/features/operations/laundry-log-manager.js`, with reusable item groups, status, totals, and filters in `js/features/operations/laundry-log-utils.js`; keep the mobile-first property handoff form in the manager and shared mismatch logic in the util layer so pending returns, record cards, and saved Firestore summaries stay aligned.
...
- Pattern: Cleaning AH stats now use comparison helpers in `js/features/operations/cleaning-ah-utils.js` (`summarizeCleaningAhPropertyRows()`, `sortCleaningAhPropertyRows()`, `summarizeCleaningAhPropertyDetail()`) while `js/features/operations/cleaning-ah-manager.js` renders the sortable property table, the stats-only cleaning-type scope, and the side-by-side focused-property drill-down; keep property-ranking rules and average-kg calculations in the util layer so filters, stats cards, and detail tables stay aligned.
...
- Pattern: colleague-specific app access now stores selected dashboard tools in `allowedEmails.allowedApps`; keep the app list in `js/shared/app-access.js`, treat privileged roles as full-access, and keep employee self-service users on time-clock-only unless explicit app keys are granted.
...
- Pattern: the Welcome Packs launcher now lives in the main landing app grid; keep using the existing `#go-to-welcome-packs-btn` id so `NavigationManager` and access-mode UI logic keep working.
...
- Pattern: Welcome Packs property charges now support multiple new-entry rows in `js/features/operations/welcome-pack-manager.js`; keep property/date/charged-amount state per row there while reusing one shared materials cart and batch persistence for stock deductions.
- Pattern: Welcome Packs property charges now support a `Quantity` field per row, allowing users to quickly log multiple identical packs for the same property/date in `js/features/operations/welcome-pack-manager.js`. `saveLog()` iterates over `quantity` to securely generate multiple individual standalone entries from a single row, decrementing stock automatically.
- Gotcha: the `welcomePack` block in `locales/pt.json` still contains legacy mojibake in some environments, so `js/features/operations/welcome-pack-manager.js` now applies a full runtime Portuguese fallback via `PT_WELCOME_PACK_TRANSLATIONS`; keep that override aligned with the English locale shape when adding new Welcome Packs copy.
- Fix: Welcome Packs Portuguese UI copy can still come from `PT_WELCOME_PACK_TRANSLATIONS` even when `locales/pt.json` is edited; if accented PT text regresses, update the fallback block in `js/features/operations/welcome-pack-manager.js` first and keep the locale entries in sync.
...
- Test: after Cleaning AH category-rule changes, save one row each for `Check-out from property owner`, `First cleaning`, and `Mid-term cleaning`; confirm owner/first skip platform commission, mid-term skips both platform commission and VAT, and the new `Stats` tab groups them under the correct category labels before rerunning `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
...
- Test: after Laundry Log changes, verify a colleague can open the landing card on mobile width, save a property handoff with outgoing counts only, reopen it from `Pending returns`, use `Copy out counts to back`, adjust one mismatch, save again, and rerun `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
...
- Test: After Cleaning AH laundry changes, manually confirm the default laundry rate shows `2,30`, switch between `Single entry` and `Batch entry`, save multiple same-day laundry rows in one batch, and use the Combined laundry register `Show` / `Order by` controls to verify the table updates correctly.
- Test: after Cleaning AH stats changes, open the `Stats` tab, switch between cleaning types in the stats-only scope, confirm the property comparison table only lists properties with matching entries for that scope, check the new average kg values, click a row to focus that property, and verify the side-by-side detail panel plus month/category/reservation breakdown update before rerunning `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
...
- Test: after Welcome Packs landing-page changes, confirm the `Welcome Packs` card appears in the main landing app grid instead of `Other Tools`, opens the same `#welcome-packs-page`, and rerun `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
- Test: after Welcome Packs property-charge changes, add at least two new rows with different properties/dates, save them together, confirm both charges appear in Calculations, and rerun `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
...
- Test: after User Management access changes, toggle app chips for an employee login, sign in as that colleague, confirm only the selected landing-page apps appear alongside Time Clock, and rerun `C:\Program Files\nodejs\node.exe tests/run-headless.mjs`.
...
- Pattern: Vehicles UI copy now lives under the shared `vehicles` locale blocks in `locales/en.json` and `locales/pt.json`; keep translations in the locale files and let `js/features/operations/vehicles-manager.js` rerender on `languageChanged` instead of hard-coding strings there.
- Test: after Vehicles changes, open the Vehicles page, switch EN/PT, and confirm the dashboard cards, filters, add/edit modal, delete confirmations, and maintenance log all update to the selected language.
- Pattern: the Operational Guide page is built from bilingual structured manual data in `js/features/operations/operational-guidelines-data.js`, with ranking/edit-merge helpers in `operational-guidelines-utils.js` and rendering in `operational-guidelines-manager.js`; browser-local edits are stored under `operational-guidelines-edits-v1` and layered over the default PDF-derived EN/PT protocols.
- Test: after changing Operational Guide content, editing, or search behavior, run `C:\Program Files\nodejs\node.exe tests/run-headless.mjs` and manually check that the landing card opens `#operational-guidelines-page`, suggested searches select the expected protocol, no-match queries show the empty state, add/edit/delete persists after refresh, `Open protocol` scrolls to the list entry, and `Copy reply` copies the approved guest response.
