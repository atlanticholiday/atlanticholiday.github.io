...
export function renderWorkEditor(property, state) {
  ...
  if (!draft) {
    ...
    return `<section><h3 class="font-semibold text-lg mb-3">${tx('workBoard')}</h3>${renderSharedStatus(state)}
      ...
      ${items.map(w=>`<article class="rr-work-item"><h4 class="font-semibold">${h(w.title)}</h4><p class="rr-note my-2">${tx(w.status)} · ${tx(`${w.category}Category`)}</p>${button('openImprovement',`data-work-id="${h(w.id)}" data-work-property="${h(w.propertyId)}" data-work-category="${h(w.category)}"`)}</article>`).join('') || `<p class="rr-note py-4">${tx('workEmpty')}</p>`}</section>`;
  }
...
export function renderReviewInbox(properties,state) {
  ...
  const extraFilters = ['property','rating','owner','work','from','to'].filter(key => filters[key] && filters[key] !== 'all').length;
  ...
  return `<section><div class="flex justify-between gap-3"><h2 class="text-xl font-semibold">${tx('reviewInbox')}</h2><span class="rr-note" role="status">${tx('results',{count:result.total})}</span></div><p class="rr-note mt-2">${tx('inboxHelp')}</p>${renderSharedStatus(state)}
    ...
    <div class="rr-controls mt-4">${select('platform','platform',[['all',t('allPlatforms')],['Airbnb','Airbnb'],['Booking.com','Booking.com'],['Direct',t('directGuest')]])}${select('response','responseStatus',[['all',t('allResponses')],['replied',t('replied')],['unanswered',t('unanswered')],['unknown',t('unknownReply')]])}${select('sort','sortReviews',[['newest',t('newest')],['priority',t('priorityReplies')]])}</div>
    <details data-rr-disclosure="inbox-filters" class="rr-advanced-filters" ${extraFilters?'open':''}><summary>${tx('moreFilters')}${extraFilters?`<span>${tx('activeFilters',{count:extraFilters})}</span>`:''}</summary><div class="rr-inbox-filters">${select('property','properties',[['all',t('allProperties')],...properties.filter(p=>!isPropertyArchived(p)).map(p=>[p.id,p.name])])}${select('rating','ratingFilter',[['all',t('allRatings')],['low',t('lowRatings')],['positive',t('positiveRatings')]])}${select('owner','owner',[['all',t('allOwners')],['unassigned',t('unassigned')],...employees(state).slice(1)])}${select('work','followUpStatus',[['all',t('allHandling')],['open',t('open')],['handled',t('handled')]])}${['from','to'].map(key=>`<label class="rr-field"><span>${tx(key==='from'?'startDate':'endDate')}</span><input type="date" id="inbox-${key}" data-inbox-filter="${key}" value="${h(filters[key])}"></label>`).join('')}</div></details>
    ${filters.from&&filters.to&&filters.from>filters.to?`<p role="alert" class="text-red-800">${tx('dateRangeError')}</p>`:''}
    ${result.rows.map(r=>`<article class="rr-inbox-row"><div><h3 class="font-semibold">${h(r.propertyName)}</h3><p class="rr-note">${h(r.platform)} · ${h(reviewDate(r.date))}</p><p class="rr-note">${h(r.author||t('guest'))} · ${h(r.score??'—')} / ${r.platform==='Airbnb'?5:10}</p></div><div><p class="rr-excerpt rr-clamp">${h([r.title,r.comment,r.positive,r.negative].filter(Boolean).join('\n') || t('noWrittenReview'))}</p><p class="rr-note mt-2">${h(statusLabel(r.replyState))} · ${tx(r.followUp?.status||'open')}${r.followUp?.assigneeName?` · ${h(r.followUp.assigneeName)}`:''}</p></div><div>${button('followUp',`data-inbox-property="${h(r.propertyId)}" data-inbox-key="${h(r.reviewKey)}"`)}</div></article>`).join('')||`<p class="rr-note py-8">${tx('inboxEmpty')}</p>`}
...
export function renderFollowUpEditor(property,state) {
  ...
  return `<section>${renderSharedStatus(state)}<h3 class="font-semibold text-lg">${tx('followUp')}</h3><p class="rr-note my-2">${h(review.author||t('guest'))} · ${h(review.platform)} · ${h(reviewDate(review.date))} · ${h(review.score??'—')} / ${review.platform==='Airbnb'?5:10}</p>
