...
export function renderYearlySummaryView(uiManager) {
    ...
    container.innerHTML = `
        <div class="schedule-year-summary space-y-6">
            <div class="schedule-year-metrics grid gap-3 grid-cols-2 xl:grid-cols-4">
                ...
                ${renderMetric(t('schedule.summary.extraHours'), totals.extraHours.toFixed(1), '')}
            </div>
            <section class="schedule-year-table overflow-hidden">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            ${rows.map((row) => `
                                <tr class="transition-colors hover:bg-slate-50/80">
                                    ...
                                    <td class="px-5 py-4 text-right font-semibold">${row.extraHours.toFixed(1)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
